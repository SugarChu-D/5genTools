...
int main() {
    ...
    uint64_t total_iterations = 100ULL * 3 * 24 * 60 * 6 * keypressManager.size();
                        ...
                        for (int second = 5; second < 11; ++second) {
                            for (size_t keypress_idx = 0; keypress_idx < keypressManager.size(); ++keypress_idx) {
                                ...
                                uint64_t current_iteration = 
                                    static_cast<uint64_t>(date_idx) * 100 * 24 * 60 * 6 * keypressManager.size() +
                                    static_cast<uint64_t>(year) * 24 * 60 * 6 * keypressManager.size() +
                                ...
                                if (target_seeds.find(upper32) != target_seeds.end()) {
                                    ...
                                    result_file << dec << 2000 + year << "/" << current_date.month << "/" << current_date.day << " "
                                                << hour << ":" << minute << ":" << second
